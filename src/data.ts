export const LOGO_URL = "https://drive.google.com/thumbnail?id=1BAW1OILqFBTITyvqebyXaYYF7pTtIF-k&sz=w1000";

export const IMAGES = {
  heroSlides: [
    {
      landscape: "https://drive.google.com/thumbnail?id=14zzL815Zj1i2YnuV71OaGR43rlfrl8GZ&sz=w1920",
      mobile: "https://drive.google.com/thumbnail?id=14zzL815Zj1i2YnuV71OaGR43rlfrl8GZ&sz=w1080"
    },
    {
      landscape: "https://drive.google.com/thumbnail?id=1j_MmOD7r0ZlLKUGQU9UloY_ywuugVoHD&sz=w1920",
      mobile: "https://drive.google.com/thumbnail?id=1KunUOvGoRzlpP5MEwY6YKkd8GWpN-ZIm&sz=w1080"
    },
    {
      landscape: "https://drive.google.com/thumbnail?id=19LwcHjLpnn4Xijr4VgeN3cM_LHQ4wSHx&sz=w1920",
      mobile: "https://drive.google.com/thumbnail?id=1PQgoq6Ncl--YSqvVN3tEynQzlme1-7dy&sz=w1080"
    },
    {
      landscape: "https://drive.google.com/thumbnail?id=1AkVxp61Nbiq7_tMPbz1RXgFVZfEBdBD2&sz=w1920",
      mobile: "https://drive.google.com/thumbnail?id=1_CxTVT8A1h5NZpKofgdft4Ia5h8uezQs&sz=w1080"
    },
    {
      landscape: "https://drive.google.com/thumbnail?id=174B5EmcC2GqUwDEi2hWkUwh2oIH39UxI&sz=w1920",
      mobile: "https://drive.google.com/thumbnail?id=174B5EmcC2GqUwDEi2hWkUwh2oIH39UxI&sz=w1080"
    }
  ],
  heroImages: [
    "https://drive.google.com/thumbnail?id=1pKCtWb4A_CPJcI4UeOBWqAnIl_XnYLfv&sz=w1920",
    "https://drive.google.com/thumbnail?id=11bTisad9hk9muFThyIgJ9YY_75byZLLO&sz=w1920",
    "https://drive.google.com/thumbnail?id=19FYB0a5sSEVcn8Vfq933oABEXC-PnnOF&sz=w1920",
    "https://drive.google.com/thumbnail?id=1QE0eedAxgy2hFSkqdGukXo6kh9vuJADM&sz=w1920",
    "https://drive.google.com/thumbnail?id=1hU2ePiAdhESb9YqX2ZPB0QAjOhGGieWy&sz=w1920",
    "https://drive.google.com/thumbnail?id=1hzPHtKvgOlLlK2VyDhNTt9fXYopmtGkB&sz=w1920",
    "https://drive.google.com/thumbnail?id=1uZQha2bv6_pDXpZZNxMKWZp-AlA_OwqT&sz=w1920",
    "https://drive.google.com/thumbnail?id=1kc5bWqA_qoaEIqB6mvMbMXdjXP5RwovB&sz=w1920",
  ],
  hero: "https://drive.google.com/thumbnail?id=1uZQha2bv6_pDXpZZNxMKWZp-AlA_OwqT&sz=w1920", // Elegant South Asian Bride / Editorial
  about: "https://drive.google.com/thumbnail?id=1PSFKJbo2m3f_mrHuifIU3iKa5WF-sNLW&sz=w1080", // Makeup artist table / Details
  experience: "https://drive.google.com/thumbnail?id=1BHK3aFtXAfIIOkMIh5ReSmNqCcz8EU7M&sz=w1080", // Soft bridal look
  destination: "https://drive.google.com/thumbnail?id=1FbLQN5cZv8JnQMoBgeR9kFlPtj7UV4Bf&sz=w1920", // Resort/Palace wedding
  contact: "https://images.unsplash.com/photo-1512314889357-e157c22f938d?fm=webp&q=75&w=1080&fit=crop", // Close up details
  portfolio: [
    "https://drive.google.com/thumbnail?id=11bTisad9hk9muFThyIgJ9YY_75byZLLO&sz=w800",
    "https://drive.google.com/thumbnail?id=1TkYZRkx6Bft1SKiE1LvKH4c3CwZdTNVo&sz=w800",
    "https://drive.google.com/thumbnail?id=173Zi3jXZkDhDghrvzhCsYUD-8mirpmAA&sz=w800",
    "https://drive.google.com/thumbnail?id=1j1jVNPVbMysukBukRXcxOTYKs2NwDgNg&sz=w800",
    "https://drive.google.com/thumbnail?id=1sl2XSexbz5oJvsiHNS5ClvZikh3vIu_g&sz=w800"
  ],
  transformations: [
    "https://drive.google.com/thumbnail?id=1SomkXXqIMtfDOQ0GS5XAlrEEsuBkFzDu&sz=w800",
    "https://drive.google.com/thumbnail?id=1u9jr6vQuVkU0BvSng5616hkemuB20O95&sz=w800",
    "https://drive.google.com/thumbnail?id=1ZpyRzgZBj8_vzgFua-Q9gBLAZisO60SW&sz=w800"
  ]
};

export const TESTIMONIALS = [
  {
    id: 1,
    name: "Aanya Reddy",
    location: "Umaid Bhawan Palace, Jodhpur",
    type: "Destination Wedding",
    text: "Niki didn't just do my makeup; she understood the mood of the entire wedding. I felt completely like myself, just elevated to a level I didn't know was possible.",
    image: IMAGES.portfolio[0]
  },
  {
    id: 2,
    name: "Priyanka Sharma",
    location: "Taj Falaknuma, Hyderabad",
    type: "Luxury Royal Wedding",
    text: "Trusting Niki was the easiest decision of my wedding planning. Her energy is so calming, and the final look was absolutely timeless. Every photo looks like a bridal editorial.",
    image: IMAGES.portfolio[1]
  },
  {
    id: 3,
    name: "Simran Kaur",
    location: "Villa del Balbianello, Lake Como",
    type: "Destination Wedding",
    text: "The airbrush technique Niki used was flawless. My makeup survived completely untouched through tears, dancing, and the Italian summer breeze. Her travel protocol was incredibly seamless.",
    image: IMAGES.portfolio[2]
  },
  {
    id: 4,
    name: "Natasha Desai",
    location: "The Oberoi Udaivilas, Udaipur",
    type: "Multi-Day Wedding",
    text: "For a multi-day event, Niki crafted distinct looks for each ceremony using her signature HD makeup. The way my skin photographed in both natural light and evening flash was simply surreal.",
    image: IMAGES.portfolio[3]
  },
  {
    id: 5,
    name: "Elena Rodriguez",
    location: "Fairmont The Palm, Dubai",
    type: "International Wedding",
    text: "Planning an international wedding is stressful, but Niki's professionalism and calming presence were unmatched. She handled different skin tones in my bridal party flawlessly.",
    image: IMAGES.portfolio[4]
  }
];
